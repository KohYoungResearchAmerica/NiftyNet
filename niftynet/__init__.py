...
class NiftynetRunner(object):
    """
    Wrapper around niftynet functionality, to allow calling from
    """
    def __init__(self, argv):
        self.argv = argv

        #ext_args = ['D:/WPy-3670-All/scripts/NiftyNet-dev/net_segment.py', 'inference', '-c', 'D:\\WPy-3670-All\\scripts\\M2\\rest\\ac_pc_seg\\acpc_segment.ini'] # sys.argv
        system_param, input_data_param = user_parameters_parser.run(self.argv)
        if util.has_bad_inputs(system_param):
            raise ValueError("has bad inputs")
        ...
        self.app_driver = driver_table[system_param['SYSTEM'].action]()
        self.app_driver.initialise_application(system_param, input_data_param)
...
def main():
    try:
        runner = NiftynetRunner(sys.argv)
